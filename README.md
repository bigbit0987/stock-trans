...
- **v2.5.2** (2026-01-09): 🎯 策略师审计增强版 (当前版本)
  - **尾盘意图识别优化**
    - 放量下跌阈值从 `-1.0%` 调整为 `-0.5%`，直接剔除（策略师建议）
    - 新增强力吸筹识别：`tail_change > 0.5%` 时加分上限提高至 15 分
  - **市场宽度自适应机制**
    - 冰点期 (breadth < 8%)：RPS 阈值动态提高至 70，只做核心标的
    - 过热期 (breadth > 30%)：启用换手率突变检测，过滤情绪过热个股
  - **RPS 动量斜率因子** (新增)
    - 计算过去 5 天 RPS 的线性回归斜率
    - `RPS > 90 且斜率为正` = 加速主升段，核心标的
    - `RPS 高但斜率为负` = 强势股退潮，减分警示
  - **差异化止盈止损**
    - Grade A：5% 回撤触发，博取大利润
    - Grade C：2% 回撤触发，"有利就走"
  - **数据库 Schema 版本控制**
    - 新增 `schema_version` 表，支持自动增量迁移
    - 避免表结构变更导致的运行时 SQL 报错
  - **Logger 异步化优化**
    - 新增 `QueueHandler` 支持，可选异步日志写入
    - 高频扫描时避免磁盘 IO 阻塞主交易逻辑
  - **pydantic-settings 环境变量约束**
    - 新增 `config/env_settings.py` 强类型配置模块
    - 自动验证 Webhook URL 格式，防止配置错误
- **v2.5.1** (2026-01-09): 🔧 架构深度优化版
  - **代码全面重构**
